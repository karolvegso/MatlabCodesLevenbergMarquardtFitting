%**************************************************************************
% This algorithm performs Powell's dog leg method fitting of Lorentz function
% with area
% author: Karol Vegso
...
clc
%**************************************************************************
% generate/simulate experimental Lorentz function with area
%**************************************************************************
% number of experimental points
M=100;
% initialize x (t_data) and y (y_data) vectors of Lorentz function
t_data=zeros(1,M);
y_data=zeros(1,M);
% initialize area
x1=107.8;
...
%**************************************************************************
% fit simulated curve
fit_result=powells_dog_leg_lorentz_area(t_data, y_data, x0, k_max, epsilon_1, epsilon_2, epsilon_3, delta);
%**************************************************************************
% function to perform Powell's dog leg fitting method on Lorentz
% function with area
...
%**************************************************************************
function [x_new] = powells_dog_leg_lorentz_area(t_data, y_data, x0, k_max, epsilon_1, epsilon_2, epsilon_3, delta)
    %**************************************************************************
    % begin of function
    %**************************************************************************
    ...
    while ((~found_bool) & (k < k_max))
        ...
        if (h_dl_norm <= epsilon_2*(x_norm+epsilon_2))
            ...
            % calculate F(x_new)
            f=zeros(1,M);
            f=y_data(1,:)-((2*x_new(1,1))/pi).*(x_new(1,3)./(4*(t_data(1,:)-x_new(1,2)).^2+x_new(1,3)^2))-x_new(1,4);
